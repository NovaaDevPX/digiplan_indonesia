  ...
  <main class="ml-64 p-10 flex-1">
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="backdrop-blur-xl bg-white/10 border border-white/20 p-6 rounded-2xl shadow-2xl mb-8">
        ...
        <p class="text-white/80">Menampilkan data pengadaan barang berdasarkan periode, status dan customer.</p>
      </div>
      ...
      <form method="GET" class="backdrop-blur-xl bg-white/10 border border-white/20 p-6 rounded-2xl shadow-2xl mb-8 grid grid-cols-1 md:grid-cols-5 gap-6">
        ...
        <div>
          ...
          <input type="date" name="tgl_awal" value="<?= $tgl_awal ?>" class="w-full p-3 bg-white/20 border border-white/30 rounded-xl text-white">
        </div>

        <div>
          <label class="block text-sm font-medium text-white/90 mb-2">Tanggal Akhir</label>
          <input type="date" name="tgl_akhir" value="<?= $tgl_akhir ?>" class="w-full p-3 bg-white/20 border border-white/30 rounded-xl text-white">
        </div>

        <div>
          <label class="block text-sm font-medium text-white/90 mb-2">Status</label>
          <select name="status" class="w-full p-3 bg-white/20 border border-white/30 rounded-xl text-white">
        ...
        <div>
          ...
          <select name="customer" class="w-full p-3 bg-white/20 border border-white/30 rounded-xl text-white">
            ...
            <?php while ($c = mysqli_fetch_assoc($customerList)): ?>
              <option value="<?= $c['id'] ?>" class="text-black" <?= ($customer == $c['id']) ? 'selected' : '' ?>>
                <?= $c['name'] ?>
              </option>
            <?php endwhile; ?>
          </select>
      ...
      <div class="flex justify-end mb-6">
        <a href="procurement-pdf.php?tgl_awal=<?= $tgl_awal ?>&tgl_akhir=<?= $tgl_akhir ?>&status=<?= $status ?>&customer=<?= $customer ?>"
          target="_blank"
          ...
          <tbody class="divide-y divide-white/10">
            ...
            <?php while ($row = mysqli_fetch_assoc($data)): ?>
              <tr class="hover:bg-white/5 transition-colors duration-200">
                ...
                <td class="p-4 text-white/90"><?= $row['customer_name'] ?></td>
                ...
                <td class="p-4 capitalize">
                  ...
                  switch ($row['status_pengadaan']) {
                    case 'diproses':
                      echo "<span class='px-3 py-1 bg-yellow-500/20 text-yellow-300 rounded-lg text-xs font-semibold border border-yellow-500/30'>Diproses</span>";
                      break;

                    case 'selesai':
                      echo "<span class='px-3 py-1 bg-emerald-500/20 text-emerald-300 rounded-lg text-xs font-semibold border border-emerald-500/30'>Selesai</span>";
                      break;

                    case 'dibatalkan':
                      echo "<span class='px-3 py-1 bg-red-500/20 text-red-300 rounded-lg text-xs font-semibold border border-red-500/30'>Dibatalkan</span>";
                      break;

                    default:
                      echo "<span class='px-3 py-1 bg-gray-500/20 text-gray-300 rounded-lg text-xs font-semibold border border-gray-500/30'>Tidak diketahui</span>";
                  }
                  ?>
                </td>
                ...
                <td class="p-4">
                  <a href="../single-report-pdf/procurement.php?kode=<?= $row['kode_pengadaan'] ?>"
                    target="_blank"
                    class="inline-flex items-center px-3 py-2 bg-gradient-to-r from-indigo-500 to-blue-600 hover:from-indigo-600 hover:to-blue-700 text-white text-xs font-semibold rounded-lg shadow-md transform hover:scale-105 transition">
